...
export function getWebSearchConfigDir(): string {
	if (process.env.DSH_WEB_ACCESS_DIR) return process.env.DSH_WEB_ACCESS_DIR;
	if (process.env.PI_CODING_AGENT_DIR) return process.env.PI_CODING_AGENT_DIR;
	if (process.env.DSH_HOME) return process.env.DSH_HOME;
	if (process.env.XDG_CONFIG_HOME) {
		const dsh = join(process.env.XDG_CONFIG_HOME, "dsh");
		const pi = join(process.env.XDG_CONFIG_HOME, "pi");
		if (existsSync(join(dsh, "web-access.json")) || existsSync(join(dsh, "web-search.json"))) return dsh;
		return pi;
	}
	const dshHome = join(homedir(), ".dsh");
	if (existsSync(join(dshHome, "web-access.json")) || existsSync(join(dshHome, "web-search.json"))) {
		return dshHome;
	}
...
export function getWebSearchConfigPath(): string {
	...
	const access = join(dir, "web-access.json");
	const search = join(dir, "web-search.json");
	if (existsSync(access)) return access;
	if (existsSync(search)) return search;
	const prefersAccess = Boolean(
		process.env.DSH_WEB_ACCESS_DIR ||
		process.env.DSH_HOME ||
		dir.endsWith(`${sep}dsh`) ||
		dir === join(homedir(), ".dsh"),
	);
	return prefersAccess ? access : search;
}
